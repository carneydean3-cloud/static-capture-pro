...
const impactColor: Record<string, string> = {
  High: "#E11D48", Medium: "#f59e0b", Low: "#A1A1AA",
};

const impactBg: Record<string, string> = {
  High: "rgba(225,29,72,0.1)", Medium: "rgba(245,158,11,0.1)", Low: "rgba(161,161,170,0.1)",
};
...
function EnquiryModal({ open, onClose, prefillUrl, prefillEmail, purchaseId }: { open: boolean; onClose: () => void; prefillUrl?: string | null; prefillEmail?: string | null; purchaseId?: string; }) {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(10,10,10,0.8)", backdropFilter: "blur(6px)" }} onClick={onClose}>
    ...
    <div className={`min-h-screen ${pageBg} px-6 py-16 transition-colors duration-500`}>
      ...
      {fullscreen && activeMockupHtml && (
        <div className="fixed inset-0 z-50 bg-black/90 flex flex-col" onClick={() => setFullscreen(false)}>
          <div className="flex items-center justify-between px-6 py-4 bg-black shrink-0 border-b border-[#27272a]" onClick={(e) => e.stopPropagation()}>
            <p className={cn("font-mono text-[10px] font-bold uppercase tracking-widest", activeColor)}>✅ Improved Direction — {mockupVersion === "a" ? "Version A (Recommended)" : "Version B (Alternative)"}</p>
            <button onClick={() => setFullscreen(false)} className="text-zinc-500 hover:text-white text-2xl leading-none">✕</button>
          </div>
