...
const FAQ = () => {
  ...
  const activeBorderHover = isGeoMode ? "hover:border-neon/40" : "hover:border-pulse/40";
  const activeIcon = isGeoMode ? "text-neon" : "text-pulse";
  ...
  return (
    <section id="faq" className="py-24 px-6 bg-obsidian border-t border-surgical">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-20">
          <span className={`font-mono text-xs font-bold uppercase tracking-widest mb-4 block ${activeColor}`}>
            FAQ
          </span>
          <h2 className="text-4xl md:text-5xl font-black mb-4 text-clinic">Got questions?</h2>
          <p className="text-lg text-data">
            {isGeoMode
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <div 
              key={index} 
              className={`bg-[#0A0A0A] border border-surgical rounded-lg overflow-hidden transition-colors duration-300 ${activeBorderHover}`}
            >
              <button
                onClick={() => setOpen(open === index ? null : index)}
                className="w-full flex items-center justify-between p-6 text-left focus:outline-none"
              >
                <span className="font-bold text-clinic text-lg pr-4">{faq.q}</span>
                <ChevronDown
                  className={`w-5 h-5 transition-transform duration-200 shrink-0 ${
                    open === index ? `rotate-180 ${activeIcon}` : "text-data"
                  }`}
                />
              </button>
              <AnimatePresence>
                {open === index && (
                  ...
                  >
                    <p className="px-6 pb-6 text-data leading-relaxed">
                      {faq.a}
                    </p>
                  </motion.div>
                )}
